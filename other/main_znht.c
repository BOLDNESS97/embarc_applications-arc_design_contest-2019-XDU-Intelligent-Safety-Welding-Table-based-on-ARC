...
/////////////////////////////////////////////////////////////////////////打开C口 控制红外感应模块的输入	
	...
	/////////////////////////////////////////////////////////////////////////打开C口 控制hongw开关（焊笔）		
	 port_time = gpio_get_dev(DW_GPIO_PORT_C);//定义GPIO C[21]
	 port_time->gpio_open(0x200000);//J4 2口
	 port_time->gpio_control(GPIO_CMD_SET_BIT_DIR_INPUT, (void *)0x200000);//设置C[21]输入
	 timer_stop(TIMER_0);//清零
 ...
 //******************************************OLED初始化*****************************
	...
	//initialize the uart0 which is on PMOD1 J3 for TXD J4 for RXD
		...
		{		
				...
				port_time->gpio_read(&tmdata, 0x200000);   //读取焊笔检测的端口J4 2口
	
			    port_huoyan->gpio_read(&hdata, 0x20000);   //读取火焰的端口J3 2口
				
				port_yanwu->gpio_read(&ywdata, 0x40000);   //读取烟雾的端口J3 3口
				
				port_hongwai->gpio_read(&hwdata, 0x80000);   //读取红外的端口 J3 4口人不在
